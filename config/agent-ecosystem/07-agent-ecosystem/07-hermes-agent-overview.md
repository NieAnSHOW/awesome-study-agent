...
Hermes Agent 的架构可以分为三个主要层次：**入口层、Agent 核心层、基础设施层**。
...
└──────────────────────┬──────────────────┬──────────────────────────┘
                       ...
                       ▼                  ▼
┌──────────────────────┐  ┌──────────────────────────────────┐
│  Session Storage     │  │  Tool Backends (工具执行后端)      │
│  (会话存储层)         │  │                                   │
│  SQLite + FTS5       │  │  Terminal (7种后端)               │
│  全文本搜索           │  │  Browser (5种后端)                 │
│  跨会话记忆          │  │  Web (4种后端)                     │
│  会话回放            │  │  MCP (动态集成)                   │
│                      │  │  File, Vision 等                  │
...
- **Gateway** (`gateway/run.py`)：消息网关，统一处理来自不同聊天平台的消息
...
**基础设施层**：
- **Session Storage**：基于 SQLite 的会话存储，支持 FTS5 全文本搜索，跨会话记忆
- **Tool Backends**：多种工具执行后端，包括终端（7 种）、浏览器（5 种）、Web 搜索等
...
├── agent/                    # 内部逻辑（提示词、上下文、记忆等）
├── hermes_cli/               # CLI 子命令和配置
├── tools/                    # 工具实现（每个文件一个工具）
├── gateway/                  # 消息网关（含 20 个平台适配器）
├── cron/                     # 定时任务调度器
├── skills/                   # 内置技能
└── tests/                    # 3000+ 测试用例
```
...
| 维度 | LangGraph | CrewAI | OpenClaw | Hermes Agent |
|------|----------|--------|---------|--------------|
| **核心范式** | 图结构工作流 | 角色驱动协作 | 多 Agent 编排 | 单 Agent 自学习 |
| **记忆机制** | 持久化状态 | 任务间依赖 | 会话记忆 | 三层记忆 + FTS5 搜索 |
| **自我改进** | 无 | 无 | 无 | 自动生成+改进 Skill |
| **多平台** | 无原生支持 | 无原生支持 | 有 | 20+ 平台统一 Gateway |
| **模型绑定** | 无 | 无 | 无 | 模型无关，运行时切换 |
| **用户画像** | 无 | 无 | 无 | 跨会话构建 |
| **部署方式** | Python 库 | Python 库 | Python 库 | 服务器端常驻服务 |
| **许可证** | MIT | MIT | MIT | MIT |
...
Hermes Agent 和之前文章介绍的其他框架有一个本质区别：**它不是一个"框架"，而是一个"Agent 本身"**。

- LangGraph 是给你工具让你搭 Agent 的框架
- CrewAI 是帮你组织多 Agent 团队的框架
- **Hermes Agent 是一个开箱即用、不断成长的 Agent**

选择使用 Hermes Agent 的决定其实很简单：**如果你希望一个 AI 越用越顺手、记得你的一切、在多个平台间无缝切换，那它就是目前开源社区里最自然的选择。**
...
[← Agent 编排](/agent-ecosystem/07-agent-ecosystem/06-orchestration) | [返回模块目录](/agent-ecosystem/07-agent-ecosystem)
