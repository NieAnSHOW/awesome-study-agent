...
本指南是 OpenClaw 的进阶学习资料。基础模块十带你"用起来"——安装、配置、连接消息平台、写一份 SOUL.md 开走你的 Agent。而本指南要回答的是：**它内部是怎么工作的？遇到生产问题怎么排查？如何做二次开发和深度定制？**

从 Gateway 进程模型到 Live Canvas 协议，从 Brain 的 LLM 编排到 Heartbeat 的心跳调度，覆盖 OpenClaw 所有核心组件的实现原理与工程细节。

## 适合读者

- 已完成基础模块十学习，希望深入理解 OpenClaw 内部机制的开发者
