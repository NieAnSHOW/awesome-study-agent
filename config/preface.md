...
**8篇文章**：AI编程概述与趋势 → Cursor → Claude Code → Windsurf/Antigravity → Codex与Kiro → Trae与国内工具 → 工具组合最佳实践 → 从零搭建实战

这块我熟。2026年的AI编程工具格局已经大变样了——Cursor 3.x 的Agent模式、Claude Code 2.1的Skills/Hooks、Windsurf被Google收购、OpenAI出了Codex CLI、字节的Trae免费可用。竞争越激烈，对咱们越有利。我的经验："Cursor做手、Claude Code做脑、Codex做腿"——不同工具打不同位置。
...
**6篇文章**：概述 → 文本转语音 → AI 音乐生成 → 声音克隆 → 商业场景 → 工具横评

ElevenLabs、Suno、Boomy——AI不只是能写字和画图了，还能说话、唱歌、作曲。从文本转语音到AI音乐生成到声音克隆，这个模块带你把这些工具全用一遍。

**检验标准**：能用至少一个 AI 音频工具生成可用的语音或音乐素材。

### AI 搜索与知识获取
**路径**：`agent-ecosystem/17-ai-search-knowledge/`
**6篇文章**：概述 → Perplexity → 国内搜索工具 → Deep Research → 搜索方法论 → 工具横评

搜索已经从"找链接"变成"直接给答案"了。Perplexity、秘塔、纳米AI、Kimi搜索——这个模块帮你建立AI时代的搜索思维，不是教你怎么搜，是教你怎么问。

**检验标准**：能用 AI 搜索工具完成一次完整的研究任务，并说清楚它与传统搜索的区别。

### 零代码 Agent 构建
**路径**：`agent-ecosystem/18-nocode-agent-building/`
**6篇文章**：概述 → Coze上手 → Dify实践 → 知识库与工作流 → 插件工具 → 完整案例

不写代码也能搭Agent。Coze、Dify、FastGPT——从"用AI"到"造AI"的思维转变。这个模块带你从零搭建一个带知识库和工作流的Agent，不需要写一行代码。

**检验标准**：能用零代码平台搭建一个具备知识库和工作流的 Agent，并成功部署。

### DeepSeek 与国产大模型
**路径**：`ai-basics/20-domestic-llm/`
**6篇文章**：背景 → DeepSeek → 通义千问 → 豆包 → Kimi与其他 → 选型指南

国产模型已经不是"平替"了，是实打实的头部玩家。DeepSeek V4/R1、通义千问、豆包、Kimi——这个模块帮你理清每家的核心优势和适用场景。这套课程就是用DeepSeek写的，你学完也该知道它多能打。

**检验标准**：能说清至少 3 个国产大模型的核心优势和适用场景，能根据需求选择合适的模型。

### AI 自动化工作流
**路径**：`agent-ecosystem/21-ai-automation-workflow/`
**6篇文章**：概述 → Zapier → n8n → Make → 流水线设计 → 个人AI工作站

整个基础模块的毕业课。Zapier、n8n、Make——把前面学的AI工具全串起来，变成7×24小时自动跑的流水线。从单独用工具到造流水线，这才是AI效率的终极形态。
